...
player_center_drawing = canvas.create_rectangle( 
    player_center_x-2, HEIGHT-player_height-5, player_center_x+2, HEIGHT-player_height+(player_height/2), fill = "black" 
    )
...
class Bullet:
    def __init__(self, canvas, x, y):
        ...
        self.bullet = canvas.create_oval(self.x_coord, self.y_coord, self.x_coord+3, self.y_coord+20, fill=self.fill)

    def move(self):
        # if not reflected, move down
        # If not reflected, move down
...
def move_enemy_bullets():
    ...
    # check for collisioms with player and enemies
...
def create_enemy(droid_type):
    ...
    if droid_type == "battle_droid":
        ...
        droid = Droid(droid_width, droid_height, canvas, random.randint(droid_width, WIDTH - droid_width), 0, speed, health, path)
        enemies.append(droid)        
    # Add more conditions for other droid types

    # Repeat the function with a delay
    droid = random.choices([d["type"] for d in droid_types], weights=[d["probability"] for d in droid_types])[0]
    delay = random.randint(4400, 6000)
    canvas.after(delay, lambda: create_enemy(droid))
